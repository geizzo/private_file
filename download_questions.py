...
class LessonTestParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: List[tuple[str, str | None]]) -> None:
        if self.collecting and tag in _HEADING_TAGS:
            if self.section_heading_level is not None and _heading_level(tag) <= self.section_heading_level:
                self.collecting = False
    ...
    def handle_endtag(self, tag: str) -> None:
        if self.in_heading and tag in _HEADING_TAGS:
            heading_text = "".join(self.heading_buffer).strip().lower()
            if "test di fine lezione" in heading_text:
                self.collecting = True
                self.section_heading_level = self.pending_heading_level
            self.in_heading = False
            self.heading_buffer.clear()
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scarica le domande della sezione 'Test di fine lezione' da una lezione LMS."
    )
    parser.add_argument("url", help="URL della pagina della lezione")
    parser.add_argument(
        "-o",
        "--output",
        metavar="FILE",
        help="Percorso del file in cui salvare le domande (oltre a stamparle).",
    )
    parser.add_argument(
        "-U",
        "--username",
        default=DEFAULT_USERNAME,
        help="Username per l'autenticazione di base.",
    )
    parser.add_argument(
        "-P",
        "--password",
        default=DEFAULT_PASSWORD,
        help="Password per l'autenticazione di base.",
    )
...
def main(argv: List[str]) -> int:
    ...
    html = fetch_html(args.url, username=args.username, password=args.password)
